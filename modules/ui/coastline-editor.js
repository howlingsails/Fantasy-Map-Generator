function editCoastline(node = d3.event.target) {
  ...
  $('#coastlineEditor').dialog({
    ...
    resizable: false,
    position: {
      my: 'center top+20', at: 'top', of: d3.event, collision: 'fit',
    },
  ...
  function drawCoastlineVertices() {
    ...
    debug.select('#vertices').selectAll('polygon').data(c).enter()
      .append('polygon')
      .attr('points', (d) => getPackPolygon(d))
      .attr('data-c', (d) => d);

    debug.select('#vertices').selectAll('circle').data(v).enter()
      .append('circle')
  ...
  function redrawCoastline() {
    ...
    const { vertices } = pack.features[f];
