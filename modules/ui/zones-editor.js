function editZones() {
  ...
  body.addEventListener('click', (ev) => {
    ...
    const zone = el.parentNode.dataset.id;
  ...
  function zonesEditorAddLines() {
    ...
    zones.selectAll('g').each(function () {
      ...
      const { description } = this.dataset;
  ...
  $(body).sortable({
    items: 'div.states', handle: '.icon-resize-vertical', containment: 'parent', axis: 'y', update: movezone,
  });
  ...
  function addZonesLayer() {
    ...
    zones.append('g').attr('id', id).attr('data-description', description).attr('data-cells', '')
      .attr('fill', fill);
    const unit = areaUnit.value === 'square' ? ` ${distanceUnitInput.value}²` : ` ${areaUnit.value}`;
