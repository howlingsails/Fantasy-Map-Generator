function editRegiment(selector) {
  ...
  function updateRegimentData(regiment) {
    ...
    composition.innerHTML = options.military.map((u) => `<div data-tip="${capitalize(u.name)} number. Input to change">
  ...
  function drawBase() {
    ...
    const base = viewbox.insert('g', 'g#armies').attr('id', 'regimentBase').attr('stroke-width', 0.3).attr('stroke', '#000')
      .attr('cursor', 'move');
    base.on('mouseenter', () => { tip('Regiment base. Drag to re-base the regiment', true); }).on('mouseleave', () => { tip('', true); });

    base.append('line').attr('x1', reg.bx).attr('y1', reg.by).attr('x2', reg.x)
      .attr('y2', reg.y)
      .attr('class', 'dragLine');
    base.append('circle').attr('cx', reg.bx).attr('cy', reg.by).attr('r', 2)
      .attr('fill', clr)
  ...
  function splitRegiment() {
    const reg = regiment(); const
      u1 = reg.u;
    const state = +elSelected.dataset.state; const
      { military } = pack.states[state];
    const i = last(military).i + 1; const
      u2 = { ...u1 }; // u clone
    ...
    const newReg = {
      a, cell: reg.cell, i, n: reg.n, u: u2, x: reg.x, y: y(reg.x, reg.y), bx: reg.bx, by: reg.by, state, icon: reg.icon,
    };
  ...
  function addRegimentOnClick() {
    ...
    const x = pack.cells.p[cell][0]; const
    ...
    const reg = {
      a: 0, cell, i, n, u: {}, x, y, bx: x, by: y, state, icon: '🛡️',
    };
  ...
  function attackRegimentOnClick() {
    ...
    const oldState = +elSelected.dataset.state; const
    ...
    // draw battle icon
    const attack = d3.transition().delay(300).duration(700).ease(d3.easeSinInOut)
  ...
  function attachRegimentOnClick() {
    ...
    const { military } = pack.states[oldState];
  ...
  function removeRegiment() {
    ...
    $('#alert').dialog({
      ...
      buttons: {
        Remove() {
          $(this).dialog('close');
          const { military } = pack.states[elSelected.dataset.state];
  ...
  function dragRegiment() {
    ...
    d3.event.on('drag', () => {
      const x = reg.x = d3.event.x; const
        y = reg.y = d3.event.y;
  ...
  function dragBase() {
    ...
    d3.event.on('end', () => { reg.bx = d3.event.x; reg.by = d3.event.y; });
  }
