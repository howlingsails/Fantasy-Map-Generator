...
function drawHeightmap() {
  ...
  terrs.append('rect').attr('x', 0).attr('y', 0).attr('width', graphWidth)
  ...
  for (const i of d3.range(20, 101)) {
    ...
    if (terracing) {
      terrs.append('path').attr('d', paths[i]).attr('transform', 'translate(.7,1.4)').attr('fill', d3.color(color).darker(terracing))
        .attr('data-height', i);
    }
...
function drawTemp() {
  ...
  for (const t of isolines) {
    ...
    const stroke = d3.color(fill).darker(0.2);
...
function drawStates() {
  ...
  const haloString = haloData
    .map(
      (d) => `<path id="state-border${d[1]}" d="${d[0]}" clip-path="url(#state-clip${d[1]})" stroke="${
...
function drawMarker(marker, rescale = 1) {
  const {
    i, icon, x, y, dx = 50, dy = 50, px = 12, size = 30, pin, fill, stroke,
  } = marker;
  const id = `marker${i}`;
...
function drawEmblems() {
  ...
  const burgCOAs = validBurgs.map((burg) => {
    ...
    return {
      type: 'burg', i: burg.i, x, y, size, shift,
    };
  });
  ...
  const provinceCOAs = validProvinces.map((province) => {
    ...
    return {
      type: 'province', i: province.i, x, y, size, shift,
    };
  });
  ...
  const stateCOAs = validStates.map((state) => {
    ...
    return {
      type: 'state', i: state.i, x, y, size, shift,
    };
  });
...
$('#mapLayers').sortable({
  items: 'li:not(.solid)', containment: 'parent', cancel: '.solid', update: moveLayer,
});
