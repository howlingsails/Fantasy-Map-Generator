...
function showElevationProfile(data, routeLen, isRiver) {
  ...
  $('#elevationProfile').dialog({
    ...
    position: {
      my: 'left top', at: 'left+20 bottom-500', of: window, collision: 'fit',
    },
  });
  ...
  let burgCount = 0;
  const chartData = {
    biome: [], burg: [], cell: [], height: [], mi: 1000000, ma: 0, mih: 100, mah: 0, points: [],
  };
  ...
  function draw() {
    ...
    chart.append('defs').append('marker').attr('id', 'arrowhead').attr('orient', 'auto')
      .attr('markerWidth', '2')
      .attr('markerHeight', '4')
      .attr('refX', '0.1')
      .attr('refY', '2')
    ...
    const landdef = chart.select('defs').append('linearGradient').attr('id', 'landdef').attr('x1', '0%')
      .attr('y1', '0%')
      .attr('x2', '0%')
      .attr('y2', '100%');
    ...
    if (chartData.mah == chartData.mih) {
      ...
      for (let k = chartData.mah; k >= chartData.mih; k--) {
        const perc = 1 - (k - chartData.mih) / (chartData.mah - chartData.mih);
        landdef
          .append('stop')
    ...
    const epCurveIndex = parseInt(epCurve.selectedIndex);
    switch (epCurveIndex) {
      case 0:
    ...
    const extra = chartData.points.slice();
    ...
    path += 'Z';
    chart.append('g').attr('id', 'epland').append('path').attr('d', path)
      .attr('stroke', 'purple')
      .attr('stroke-width', '0')
    ...
    for (let k = 0; k < chartData.points.length; k++) {
      ...
      g.append('rect').attr('stroke', c).attr('fill', c).attr('x', x)
        .attr('y', y)
        .attr('width', xscale(1))
        .attr('height', 15)
    ...
    const xAxis = d3
      .axisBottom(xscale)
      .ticks(10)
      .tickFormat((d) => `${rn((d / chartData.points.length) * routeLen)} ${distanceUnitInput.value}`);
    const yAxis = d3
      .axisLeft(yscale)
      .ticks(5)
      .tickFormat((d) => `${d} ${hu}`);
    ...
    chart
      ...
      .attr('transform', (d) => 'rotate(0)', // used to rotate labels, - anti-clockwise, + clockwise
      );
    ...
    const xwidth = chartData.points[1][0] - chartData.points[0][0];
    for (let k = 0; k < chartData.points.length; k++) {
