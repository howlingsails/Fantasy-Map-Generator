...
styleElements.addEventListener('change', (ev) => {
...
styleRadiusPlus.addEventListener('click', () => {
  const size = Math.max(rn(getEl().attr('size') * 1.1, 2), 0.2);
  changeRadius(size);
});

styleRadiusMinus.addEventListener('click', () => {
...
styleIconSizePlus.addEventListener('click', () => {
  const size = Math.max(rn(getEl().attr('size') * 1.1, 2), 0.2);
  changeIconSize(size);
});

styleIconSizeMinus.addEventListener('click', () => {
  const size = Math.max(rn(getEl().attr('size') * 0.9, 2), 0.2);
  changeIconSize(size);
});
...
function applyDefaultStyle() {
  armies.attr('opacity', 1).attr('fill-opacity', 1).attr('font-size', 6).attr('box-size', 3)
    .attr('stroke', '#000')
    .attr('stroke-width', 0.3);

  biomes.attr('opacity', null).attr('filter', null).attr('mask', 'url(#land)');
  ice.attr('opacity', 0.9).attr('fill', '#e8f0f6').attr('stroke', '#e8f0f6').attr('stroke-width', 1)
  ...
  provinceBorders
    ...
    .attr('filter', null);
  cells.attr('opacity', null).attr('stroke', '#808080').attr('stroke-width', 0.1).attr('filter', null)
    .attr('mask', null);
  ...
  coordinates
    ...
    .attr('filter', null)
    .attr('mask', null);
  compass.attr('opacity', 0.8).attr('transform', null).attr('filter', null).attr('mask', 'url(#water)')
  ...
  prec.attr('opacity', null).attr('stroke', '#000000').attr('stroke-width', 0.1).attr('fill', '#003dff')
    .attr('filter', null);
  population.attr('opacity', null).attr('stroke-width', 1.6).attr('stroke-dasharray', null).attr('stroke-linecap', 'butt')
    .attr('filter', null);
  population.select('#rural').attr('stroke', '#0000ff');
  population.select('#urban').attr('stroke', '#ff0000');

  lakes.select('#freshwater').attr('opacity', 0.5).attr('fill', '#a6c1fd').attr('stroke', '#5f799d')
    .attr('stroke-width', 0.7)
    .attr('filter', null);
  lakes.select('#salt').attr('opacity', 0.5).attr('fill', '#409b8a').attr('stroke', '#388985')
    .attr('stroke-width', 0.7)
    .attr('filter', null);
  lakes.select('#sinkhole').attr('opacity', 1).attr('fill', '#5bc9fd').attr('stroke', '#53a3b0')
    .attr('stroke-width', 0.7)
    .attr('filter', null);
  lakes.select('#frozen').attr('opacity', 0.95).attr('fill', '#cdd4e7').attr('stroke', '#cfe0eb')
    .attr('stroke-width', 0)
    .attr('filter', null);
  lakes.select('#lava').attr('opacity', 0.7).attr('fill', '#90270d').attr('stroke', '#f93e0c')
    .attr('stroke-width', 2)
    .attr('filter', 'url(#crumpled)');
  lakes.select('#dry').attr('opacity', 1).attr('fill', '#c9bfa7').attr('stroke', '#8e816f')
    .attr('stroke-width', 0.7)
    .attr('filter', null);
  ...
  coastline.select('#lake_island').attr('opacity', 1).attr('stroke', '#7c8eaf').attr('stroke-width', 0.35)
    .attr('filter', null);

  terrain.attr('opacity', null).attr('set', 'simple').attr('size', 1).attr('density', 0.4)
    .attr('filter', null)
    .attr('mask', null);
  ...
  provs.attr('opacity', 0.7).attr('fill', '#000000').attr('font-family', 'Georgia').attr('data-size', 10)
    .attr('font-size', 10)
    .attr('filter', null);
  ...
  burgIcons
    ...
    .attr('stroke-linecap', 'butt');
  anchors.select('#cities').attr('opacity', 1).attr('fill', '#ffffff').attr('stroke', '#3e3e4b')
  ...
  burgIcons
    ...
    .attr('stroke-dasharray', '')
    .attr('stroke-linecap', 'butt');
  anchors.select('#towns').attr('opacity', 1).attr('fill', '#ffffff').attr('stroke', '#3e3e4b')
    .attr('stroke-width', 1.2)
    .attr('size', 1);
  ...
  anchors.selectAll('g').each(function (d) {
    ...
    d3.select(this)
      ...
      .each(function () {
        ...
        const { x } = pack.burgs[id];
        const { y } = pack.burgs[id];
