function editReligions() {
  ...
  function regenerateDeity() {
    const religion = +this.parentNode.dataset.id;
    const { culture } = pack.religions[religion];
    ...
    $('#alert').dialog({
      ...
      buttons: {
        Remove() {
          ...
          const { origin } = pack.religions[religion];
  ...
  function drawReligionCenters() {
    ...
    const religionCenters = debug.append('g').attr('id', 'religionCenters').attr('stroke-width', 1.2).attr('stroke', '#444444')
      .style('cursor', 'move');
  ...
  function showHierarchy() {
    ...
    const svg = d3.select('#alertMessage').insert('svg', '#religionInfo').attr('id', 'hierarchy').attr('width', width)
      .attr('height', height)
    ...
    function renderTree() {
      ...
      links
        ...
        .enter()
        .append('path')
        .attr('d', (d) => `M${d.source.x},${d.source.y}C${d.source.x},${(d.source.y * 3 + d.target.y) / 4} ${d.target.x},${(d.source.y * 2 + d.target.y) / 3} ${d.target.x},${d.target.y}`);
      ...
      node
        ...
        .attr('d', (d) => {
          ...
          // circle
          if (d.data.type === 'Heresy') return 'M0,-14L14,0L0,14L-14,0Z';
          // diamond
          if (d.data.type === 'Cult') return 'M-6.5,-11.26l13,0l6.5,11.26l-6.5,11.26l-13,0l-6.5,-11.26Z';
          // hex
  ...
  function changeReligionForSelection(selection) {
    ...
    selection.forEach((i) => {
      ...
      if (exists.size()) exists.attr('data-religion', r).attr('fill', color);
        ...
        temp.append('polygon').attr('data-cell', i).attr('data-religion', r).attr('points', getPackPolygon(i))
          .attr('fill', color);
      }
    });
  }
