function editDiplomacy() {
  ...
  function stateHighlightOn(event) {
    ...
    const l = path.node().getTotalLength(); const
      dur = (l + 5000) / 2;
    const i = d3.interpolateString(`0,${l}`, `${l},${l}`);
    path.transition().duration(dur).attrTween('stroke-dasharray', () => (t) => i(t));
  }
  ...
  function diplomacyChangeRelations(event) {
    ...
    if (rel === 'Rival') chronicle.push(rival()); else { chronicle.push(change()); }
  ...
  function showRelationsHistory() {
    ...
    $('#alert').dialog({
      ...
      buttons: {
        ...
        Clear() { pack.states[0].diplomacy = []; $(this).dialog('close'); },
  ...
  function showRelationsMatrix() {
    ...
    $('#alert').dialog({
      title: 'Relations matrix', width: fitContent(), position: { my: 'center', at: 'center', of: 'svg' }, buttons: {},
    });
  }
