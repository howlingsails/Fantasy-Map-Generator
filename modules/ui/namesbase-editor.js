function editNamesbase() {
  ...
  function analizeNamesbase() {
    ...
    const lengthStat = l < 30 ? "<span style='color:red'>[not enough]</span>"
    ...
    const rangeStat = l < 10 ? "<span style='color:red'>[low]</span>"
      : l < 15 ? "<span style='color:darkred'>[low]</span>"
        : l < 20 ? "<span style='color:orange'>[low]</span>"
          : "<span style='color:green'>[good]</span>";

    const depthStat = l < 15 ? "<span style='color:red'>[low]</span>"
  ...
  function namesbaseAdd() {
    ...
    nameBases.push({
      name: `Base${base}`, min: 5, max: 12, d: '', m: 0, b,
    });
  ...
  function namesbaseUpload(dataLoaded) {
    ...
    data.forEach((d) => {
      const e = d.split('|');
      nameBases.push({
        name: e[0], min: e[1], max: e[2], d: e[3], m: e[4], b: e[5],
      });
    });
