function overviewBurgs() {
  ...
  function showBurgsChart() {
    ...
    const states = pack.states.map((s) => {
      ...
      return {
        id: s.i, state: s.i ? 0 : null, color, name,
      };
    });
    ...
    const burgs = pack.burgs
      ...
      .map((b) => {
        ...
        return {
          id, i: b.i, state: b.state, culture: b.culture, province, parent, name: b.name, population, capital, x: b.x, y: b.y,
        };
      });
    ...
    const margin = {
      top: 0, right: -50, bottom: -10, left: -50,
    };
    ...
    function updateChart() {
      ...
      const getCulturesData = () => pack.cultures.map((c) => {
        ...
        return {
          id: c.i, culture: c.i ? 0 : null, color, name: c.name,
        };
      });
      ...
      const getParentData = () => {
        const states = pack.states.map((s) => {
          ...
          const name = s.fullName ? s.fullName : s.name;
          return {
            id: s.i, parent: s.i ? 0 : null, color, name,
          };
        });
        const provinces = pack.provinces
          .filter((p) => p.i && !p.removed)
          .map((p) => ({
            id: p.i + states.length - 1, parent: p.state, color: p.color, name: p.fullName,
          }));
      ...
      const getProvincesData = () => pack.provinces.map((p) => {
        ...
        return {
          id: p.i ? p.i : 0, province: p.i ? 0 : null, color, name,
        };
      });
  ...
  function downloadBurgsData() {
    let data = `Id,Burg,Province,Province Full Name,State,State Full Name,Culture,Religion,Population,Longitude,Latitude,Elevation (${
      heightUnit.value
