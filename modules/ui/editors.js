...
function addBurg(point) {
  ...
  pack.burgs.push({
    name, cell, x, y, state, i, culture, feature, capital: 0, port: 0, temple, population, coa, type,
  });
  cells.burg[cell] = i;
...
function createPicker() {
  ...
  const contaiter = d3.select('body').append('svg').attr('id', 'pickerContainer').attr('width', '100%')
    .attr('height', '100%');
  contaiter
    .append('rect')
    .attr('x', 0)
  ...
  h.append('line').attr('x1', 18).attr('y1', 10).attr('x2', 107)
    .attr('y2', 10);
  h.append('circle').attr('cx', 75).attr('cy', 10).attr('r', 5)
    .attr('id', 'pickerH');
  h.on('mousemove', () => tip('Set palette hue'));

  const s = controls.append('g');
  s.append('text').attr('x', 113).attr('y', 14).text('S:');
  s.append('line').attr('x1', 124).attr('y1', 10).attr('x2', 206)
    .attr('y2', 10);
  s.append('circle').attr('cx', 181.4).attr('cy', 10).attr('r', 5)
    .attr('id', 'pickerS');
  s.on('mousemove', () => tip('Set palette saturation'));

  const l = controls.append('g');
  l.append('text').attr('x', 213).attr('y', 14).text('L:');
  l.append('line').attr('x1', 226).attr('y1', 10).attr('x2', 306)
    .attr('y2', 10);
  l.append('circle').attr('cx', 282).attr('cy', 10).attr('r', 5)
    .attr('id', 'pickerL');
  ...
  picker
    ...
    .attr('stroke', '#5d4651')
    .on('mousemove', pos);
  picker.insert('text', ':first-child').attr('x', 291).attr('y', -10).attr('id', 'pickerCloseText')
    .text('✕');
  picker
    .insert('rect', ':first-child')
    .attr('x', 288)
    ...
    .on('mousemove', cl)
    .on('click', closePicker);
  picker.insert('text', ':first-child').attr('x', 12).attr('y', -10).attr('id', 'pickerLabel')
    .text('Color Picker')
    .on('mousemove', pos);
  picker.insert('rect', ':first-child').attr('x', 0).attr('y', -30).attr('width', width)
    .attr('height', 30)
...
function fog(id, path) {
  ...
  if (defs.select('#fog path').size()) {
    defs.select('#fog').append('path').attr('d', path).attr('id', id)
      .attr('opacity', 0)
      .transition(fadeIn)
      .attr('opacity', 1);
  } else {
...
function highlightElement(element, zoom) {
  ...
  highlight.transition(enter).style('outline-offset', '0px').transition(exit).style('outline-color', 'transparent')
    .delay(1000)
    .remove();
