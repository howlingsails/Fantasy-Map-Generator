function editStates() {
  ...
  function editStateName(state) {
    ...
    function regenerateShortNameCuture() {
      ...
      const { culture } = pack.states[state];
    ...
    function addCustomForm() {
      const { value } = stateNameEditorCustomForm;
      const addModeActive = stateNameEditorCustomForm.style.display === 'inline-block';
  ...
  function stateChangeCapitalName(state, line, value) {
    ...
    const { capital } = pack.states[state];
    if (!capital) return;
  ...
  function changeStateForSelection(selection) {
    ...
    selection.forEach((i) => {
      ...
      if (exists.size()) exists.attr('data-state', stateNew).attr('fill', color).attr('stroke', color);
        ...
        temp.append('polygon').attr('data-cell', i).attr('data-state', stateNew).attr('points', getPackPolygon(i))
          .attr('fill', color)
          .attr('stroke', color);
      }
    });
  ...
  function adjustProvinces(affectedProvinces) {
    ...
    function createProvince(oldProvince, stateId, provinceCells) {
      ...
      provinces.push({
        i: newProvinceId, state: stateId, center, burg: burgId, name, formName, fullName: `${name} ${formName}`, color, coa,
      });
    ...
    function findClosestProvince(provinceId, stateId, sourceCells) {
      const borderCell = sourceCells.find((i) => cells.c[i].some((c) => cells.state[c] === stateId && cells.province[c] && cells.province[c] !== provinceId));
