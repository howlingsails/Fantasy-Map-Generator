...
function showSupporters() {
  ...
  $('#alert').dialog({
    resizable: false, title: 'Patreon Supporters', width: '54vw', position: { my: 'center', at: 'center', of: 'svg' },
  });
}
...
function changeMapSize() {
  ...
  landmass.select('rect').attr('x', 0).attr('y', 0).attr('width', maxWidth)
    .attr('height', maxHeight);
  oceanPattern.select('rect').attr('x', 0).attr('y', 0).attr('width', maxWidth)
    .attr('height', maxHeight);
  oceanLayers.select('rect').attr('x', 0).attr('y', 0).attr('width', maxWidth)
    .attr('height', maxHeight);
  fogging.selectAll('rect').attr('x', 0).attr('y', 0).attr('width', maxWidth)
    .attr('height', maxHeight);
...
function changeCultureSet() {
  const { max } = culturesSet.selectedOptions[0].dataset;
...
function randomizeOptions() {
  ...
  const tMax = 30;
  const tMin = -30; // temperature extremes
...
document.getElementById('sticked').addEventListener('click', (event) => {
  const { id } = event.target;
...
function toggle3dOptions() {
  ...
  $('#options3d').dialog({
    ...
    width: fitContent(),
    position: {
      my: 'right top', at: 'right-30 top+10', of: 'svg', collision: 'fit',
    },
  });
