function overviewRegiments(state) {
  ...
  function updateHeaders() {
    ...
    header.querySelectorAll('.removable').forEach((e) => {
  ...
  function togglePercentageMode() {
    if (body.dataset.type === 'absolute') {
      ...
      const array = Array.from(lines); const
        cache = [];
  ...
  function addRegimentOnClick() {
    ...
    const { military } = pack.states[state];
  ...
  function downloadRegimentsData() {
    ...
    body.querySelectorAll(':scope > div:not(.totalLine)').forEach((el) => {
