...
function recalculatePopulation() {
  ...
  pack.burgs.forEach((b) => {
    ...
    if (b.capital) b.population *= 1.3; // increase capital population
    if (b.port) b.population *= 1.3; // increase port population
...
function regenerateStates() {
  ...
  pack.states = d3.range(count).map((i) => {
    ...
    return {
      i, name, type, capital: capital.i, center: capital.cell, culture, expansionism, coa,
    };
  });
...
function regenerateBurgs() {
  ...
  for (let i = 0; i < sorted.length && burgs.length < burgsCount; i++) {
    ...
    burgs.push({
      cell, x, y, state, i: id, culture, name, capital, feature: cells.f[cell],
    });
    burgsTree.add([x, y]);
    cells.burg[cell] = id;
  }
...
function regenerateZones(event) {
  if (isCtrlClick(event)) {
    prompt('Please provide zones number multiplier', {
...
function addLabelOnClick() {
  ...
  const selected = labelGroupSelect.value;
  ...
  const { width } = example.node().getBBox();
...
function addRiverOnClick() {
  ...
  const {
    alterHeights, resolveDepressions, addMeandering, getRiverPath, getBasin, getName, getType, getWidth, getOffset, getApproximateLength,
  } = Rivers;
  const riverCells = [];
  ...
  if (river) {
    ...
    rivers.push({
      i: riverId, source, mouth, discharge, length, width, widthFactor, sourceWidth: 0, parent, cells: riverCells, basin, name, type,
    });
  }
...
function addRouteOnClick() {
  ...
  elSelected = routes.select('g').append('path').attr('id', id).attr('data-new', 1)
...
function configMarkersGeneration() {
  ...
  const applyChanges = () => {
    ...
    const newConfig = config.map((markerType, index) => {
      ...
      return {
        ...markerType, type, icon, multiplier,
      };
    });
  ...
  $('#alert').dialog({
    ...
    position: {
      my: 'left top', at: 'left+10 top+10', of: 'svg', collision: 'fit',
    },
    buttons: {
      Regenerate: () => {
        applyChanges();
...
function viewCellDetails() {
  $('#cellInfo').dialog({
    ...
    title: 'Cell Details',
    position: {
      my: 'right top', at: 'right-10 top+10', of: 'svg', collision: 'fit',
    },
  });
}
