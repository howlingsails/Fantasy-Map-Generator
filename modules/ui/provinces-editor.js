function editProvinces() {
  ...
  function editProvinceName(province) {
    ...
    function addCustomForm() {
      const { value } = provinceNameEditorCustomForm;
  ...
  function showChart() {
    ...
    const height = 90 + 90 * uiSizeOutput.value;
    const margin = {
    ...
    function showInfo(ev, d) {
      ...
      const value = provincesTreeType.value === 'area'
        ? `Area: ${area}`
  ...
  function enterProvincesManualAssignent() {
    ...
    provs.select('g#provincesBody').append('g').attr('id', 'centers').attr('fill', 'none')
      .attr('stroke', '#ff0000')
      .attr('stroke-width', 1);
  ...
  function changeForSelection(selection) {
    ...
    const { state } = pack.provinces[provinceNew];
  ...
  function addProvince() {
    ...
    provinces.push({
      i: province, state, center, burg, name, formName, fullName, color, coa,
    });
