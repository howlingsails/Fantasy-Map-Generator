function editLabel() {
  ...
  $('#labelEditor').dialog({
    ...
    width: fitContent(),
    position: {
      my: 'center top+10', at: 'bottom', of: text, collision: 'fit',
    },
  ...
  function addControlPoint(point) {
    debug.select('#controlPoints').append('circle').attr('cx', point.x).attr('cy', point.y)
      .attr('r', 2.5)
  ...
  function addInterimControlPoint() {
    ...
    debug.select('#controlPoints').insert('circle', before).attr('cx', point[0]).attr('cy', point[1])
      .attr('r', 2.5)
      .attr('stroke-width', 0.8)
      .call(d3.drag().on('drag', dragControlPoint))
      .on('click', clickControlPoint);
  ...
  function changeText() {
    ...
    const example = d3.select(elSelected.node().parentNode).append('text').attr('x', 0).attr('x', 0)
