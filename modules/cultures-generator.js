window.Cultures = (function () {
  ...
  const generate = function () {
    ...
    function placeCenter(v) {
      let c;
      let spacing = (graphWidth + graphHeight) / 20 / count;
    ...
    cultures.forEach((c) => (c.base %= nameBases.length));

    function selectCultures(c) {
      const def = getDefault(c);
  ...
  const add = function (center) {
    ...
    pack.cultures.push({
      name, color, base, center, i, expansionism: 1, type: 'Generic', cells: 0, area: 0, rural: 0, urban: 0, origin: 0, code, shield,
    });
  };
  ...
  const getDefault = function (count) {
    ...
    const { s } = cells;
    const sMax = d3.max(s);
    const temperature = cells.t; // Temperature
    const height = cells.h; // Height
    ...
    if (culturesSet.value === 'darkFantasy') {
      return [
        {
          name: 'Castien (Elvish)', base: 33, odd: 1, sort: (i) => (normalizedCellScore(i) / biomeGoals(i, [6, 7, 8, 9], 10)) * temperature[i], shield: 'gondor',
        }, // Elves
        {
          name: 'Hagluin (Elvish)', base: 33, odd: 1, sort: (i) => (normalizedCellScore(i) / biomeGoals(i, [6, 7, 8, 9], 10)) * temperature[i], shield: 'noldor',
        }, // Elves
        {
          name: 'Lothian (Elvish)', base: 33, odd: 1, sort: (i) => (normalizedCellScore(i) / biomeGoals(i, [7, 8, 9, 12], 10)) * temperature[i], shield: 'wedged',
        },
        {
          name: 'Dunirr (Dwarven)', base: 35, odd: 1, sort: (i) => normalizedCellScore(i) + (height[i] * 3), shield: 'ironHills',
        }, // Dwarfs
        {
          name: 'Khazadur (Dwarven)', base: 35, odd: 1, sort: (i) => normalizedCellScore(i) + (height[i] * 4), shield: 'erebor',
        }, // Dwarfs
        {
          name: 'Dhommeam (Dwarven)', base: 35, odd: 1, sort: (i) => normalizedCellScore(i) + height[i], shield: 'erebor',
        }, // Dwarfs
        {
          name: 'Mudtoe (Dwarven)', base: 35, odd: 1, sort: (i) => normalizedCellScore(i) + height[i], shield: 'erebor',
        }, // Dwarfs
        {
          name: 'Gongrem (Dwarven)', base: 35, odd: 1, sort: (i) => normalizedCellScore(i) + height[i], shield: 'erebor',
        }, // Dwarfs
        {
          name: 'Pabolk (Goblin)', base: 36, odd: 1, sort: (i) => temperature[i] - s[i], shield: 'moriaOrc',
        }, // Goblin
        {
          name: 'Lagakh (Orkish)', base: 37, odd: 1, sort: (i) => (height[i] * normalizedCellScore(i) * biomeGoals(i, [2], 100)), shield: 'urukHai',
        }, // Orc
        {
          name: 'Mogak (Orkish)', base: 37, odd: 1, sort: (i) => (height[i] * normalizedCellScore(i) * biomeGoals(i, [2], 100)), shield: 'urukHai',
        }, // Orc
        {
          name: 'Xugarf (Orkish)', base: 37, odd: 1, sort: (i) => ((height[i] * normalizedCellScore(i) * temperature[i]) / biomeGoals(i, [2, 10, 11], 100)), shield: 'moriaOrc',
        }, // Orc
        {
          name: 'Zildud (Orkish)', base: 37, odd: 1, sort: (i) => ((height[i] * normalizedCellScore(i) * temperature[i]) / biomeGoals(i, [2, 10, 11], 100)), shield: 'moriaOrc',
        }, // Orc
        {
          name: 'Shazgob (Orkish)', base: 37, odd: 1, sort: (i) => ((height[i] * normalizedCellScore(i) * temperature[i]) / biomeGoals(i, [2, 10, 11], 100)), shield: 'moriaOrc',
        }, // Orc
        {
          name: 'Mazoga (Orkish)', base: 37, odd: 1, sort: (i) => ((height[i] * temperature[i]) / biomeGoals(i, [2, 10, 11], 100)), shield: 'moriaOrc',
        }, // Orc
        {
          name: 'Gul (Orkish)', base: 37, odd: 1, sort: (i) => ((height[i] * temperature[i]) / biomeGoals(i, [2, 10, 11], 100)), shield: 'moriaOrc',
        }, // Orc
        {
          name: 'Goren (Elvish}', base: 33, odd: 0.5, sort: (i) => (normalizedCellScore(i) / biomeGoals(i, [6, 7, 8, 9], 10)) * temperature[i], shield: 'fantasy5',
        }, // Elves
        {
          name: 'Ginikirr {Dwarven)', base: 35, odd: 0.8, sort: (i) => normalizedCellScore(i) + height[i], shield: 'erebor',
        }, // Dwarven
        {
          name: 'Heenzurm (Goblin)', base: 36, odd: 0.8, sort: (i) => temperature[i] - s[i], shield: 'moriaOrc',
        }, // Goblin
        {
          name: 'Yotunn (Giant)', base: 38, odd: 0.8, sort: (i) => tempDiff(i, -5), shield: 'pavise',
        }, // Giant
        {
          name: 'Zakaos (Giant)', base: 38, odd: 0.8, sort: (i) => tempDiff(i, -5), shield: 'pavise',
        }, // Giant
        {
      ...
      if (culturesSet.value === 'antique') {
        return [
          {
            name: 'Roman', base: 8, odd: 1, sort: (i) => normalizedCellScore(i) / tempDiff(i, 14) / temperature[i], shield: 'roman',
          }, // Roman
          {
            name: 'Roman', base: 8, odd: 1, sort: (i) => normalizedCellScore(i) / tempDiff(i, 15) / sf(i), shield: 'roman',
          }, // Roman
          {
            name: 'Roman', base: 8, odd: 1, sort: (i) => normalizedCellScore(i) / tempDiff(i, 16) / sf(i), shield: 'roman',
          }, // Roman
          {
            name: 'Roman', base: 8, odd: 1, sort: (i) => normalizedCellScore(i) / tempDiff(i, 17) / temperature[i], shield: 'roman',
          }, // Roman
          {
            name: 'Hellenic', base: 7, odd: 1, sort: (i) => (normalizedCellScore(i) / tempDiff(i, 18) / sf(i)) * height[i], shield: 'boeotian',
          }, // Greek
          {
            name: 'Hellenic', base: 7, odd: 1, sort: (i) => (normalizedCellScore(i) / tempDiff(i, 19) / sf(i)) * height[i], shield: 'boeotian',
          }, // Greek
          {
            name: 'Macedonian', base: 7, odd: 0.5, sort: (i) => (normalizedCellScore(i) / tempDiff(i, 12)) * height[i], shield: 'round',
          }, // Greek
          {
          ...
          {
            name: 'Persian', base: 24, odd: 0.8, sort: (i) => (normalizedCellScore(i) / tempDiff(i, 18)) * height[i], shield: 'oval',
          }, // Iranian
          {
            name: 'Scythian', base: 24, odd: 0.5, sort: (i) => normalizedCellScore(i) / tempDiff(i, 11) ** 0.5 / biomeGoals(i, [4]), shield: 'round',
          }, // Iranian
          {
            name: 'Cantabrian', base: 20, odd: 0.5, sort: (i) => (normalizedCellScore(i) / tempDiff(i, 16)) * height[i], shield: 'oval',
          }, // Basque
          {
            name: 'Estian', base: 9, odd: 0.2, sort: (i) => (normalizedCellScore(i) / tempDiff(i, 5)) * temperature[i], shield: 'pavise',
          }, // Finnic
          {
            name: 'Carthaginian', base: 17, odd: 0.3, sort: (i) => normalizedCellScore(i) / tempDiff(i, 19) / sf(i), shield: 'oval',
          }, // Berber
          {
            name: 'Mesopotamian', base: 23, odd: 0.2, sort: (i) => normalizedCellScore(i) / tempDiff(i, 22) / biomeGoals(i, [1, 2, 3]), shield: 'oval',
          }, // Mesopotamian
        ];
      }
      ...
      if (culturesSet.value === 'highFantasy') {
        ...
        // fantasy races
          {
            name: 'Quenian (Elfish)', base: 33, odd: 1, sort: (i) => (normalizedCellScore(i) / biomeGoals(i, [6, 7, 8, 9], 10)) * temperature[i], shield: 'gondor',
          }, // Elves
          {
            name: 'Eldar (Elfish)', base: 33, odd: 1, sort: (i) => (normalizedCellScore(i) / biomeGoals(i, [6, 7, 8, 9], 10)) * temperature[i], shield: 'noldor',
          }, // Elves
          {
            name: 'Trow (Dark Elfish)', base: 34, odd: 0.9, sort: (i) => (normalizedCellScore(i) / biomeGoals(i, [7, 8, 9, 12], 10)) * temperature[i], shield: 'hessen',
          }, // Dark Elves
          {
            name: 'Lothian (Dark Elfish)', base: 34, odd: 0.3, sort: (i) => (normalizedCellScore(i) / biomeGoals(i, [7, 8, 9, 12], 10)) * temperature[i], shield: 'wedged',
          }, // Dark Elves
          {
            name: 'Dunirr (Dwarven)', base: 35, odd: 1, sort: (i) => normalizedCellScore(i) + height[i], shield: 'ironHills',
          }, // Dwarfs
          {
            name: 'Khazadur (Dwarven)', base: 35, odd: 1, sort: (i) => normalizedCellScore(i) + height[i], shield: 'erebor',
          }, // Dwarfs
          {
            name: 'Kobold (Goblin)', base: 36, odd: 1, sort: (i) => temperature[i] - s[i], shield: 'moriaOrc',
          }, // Goblin
          {
            name: 'Uruk (Orkish)', base: 37, odd: 1, sort: (i) => height[i] * temperature[i], shield: 'urukHai',
          }, // Orc
          {
            name: 'Ugluk (Orkish)', base: 37, odd: 1.5, sort: (i) => (height[i] * temperature[i]) / biomeGoals(i, [1, 2, 10, 11]), shield: 'moriaOrc',
          }, // Orc
          {
            name: 'Yotunn (Giants)', base: 38, odd: 0.7, sort: (i) => tempDiff(i, -10), shield: 'pavise',
          }, // Giant
          {
            name: 'Rake (Drakonic)', base: 39, odd: 0.7, sort: (i) => -s[i], shield: 'fantasy2',
          }, // Draconic
          {
            name: 'Arago (Arachnid)', base: 40, odd: 0.7, sort: (i) => temperature[i] - s[i], shield: 'horsehead2',
          }, // Arachnid
          {
            name: "Aj'Snaga (Serpents)", base: 41, odd: 0.7, sort: (i) => normalizedCellScore(i) / biomeGoals(i, [12], 10), shield: 'fantasy1',
          }, // Serpents
          // fantasy human
          {
    ...
    if (culturesSet.value === 'random') {
      return d3.range(count).map(() => {
        ...
        return {
          name, base: rnd, odd: 1, shield: getRandomShield(),
        };
      });
    }
  ...
  return {
    generate, add, expand, getDefault, getRandomShield,
  };
}());
