window.Military = (function () {
  const generate = function () {
    ...
    const rate = {
      x: 0, Ally: -0.2, Friendly: -0.1, Neutral: 0, Suspicion: 0.1, Enemy: 1, Unknown: 0, Rival: 0.5, Vassal: 0.5, Suzerain: -0.5,
    };

    const stateModifier = {
      melee: {
        Nomadic: 0.5, Highland: 1.2, Lake: 1, Naval: 0.7, Hunting: 1.2, River: 1.1,
      },
      ranged: {
        Nomadic: 0.9, Highland: 1.3, Lake: 1, Naval: 0.8, Hunting: 2, River: 0.8,
      },
      mounted: {
        Nomadic: 2.3, Highland: 0.6, Lake: 0.7, Naval: 0.3, Hunting: 0.7, River: 0.8,
      },
      machinery: {
        Nomadic: 0.8, Highland: 1.4, Lake: 1.1, Naval: 1.4, Hunting: 0.4, River: 1.1,
      },
      naval: {
        Nomadic: 0.5, Highland: 0.5, Lake: 1.2, Naval: 1.8, Hunting: 0.7, River: 1.2,
      },
      armored: {
        Nomadic: 1, Highland: 0.5, Lake: 1, Naval: 1, Hunting: 0.7, River: 1.1,
      },
      aviation: {
        Nomadic: 0.5, Highland: 0.5, Lake: 1.2, Naval: 1.2, Hunting: 0.6, River: 1.2,
      },
      magical: {
        Nomadic: 1, Highland: 2, Lake: 1, Naval: 1, Hunting: 1, River: 1,
      },
    };

    const cellTypeModifier = {
      nomadic: {
        melee: 0.2, ranged: 0.5, mounted: 3, machinery: 0.4, naval: 0.3, armored: 1.6, aviation: 1, magical: 0.5,
      },
      wetland: {
        melee: 0.8, ranged: 2, mounted: 0.3, machinery: 1.2, naval: 1.0, armored: 0.2, aviation: 0.5, magical: 0.5,
      },
      highland: {
        melee: 1.2, ranged: 1.6, mounted: 0.3, machinery: 3, naval: 1.0, armored: 0.8, aviation: 0.3, magical: 2,
      },
    };

    const burgTypeModifier = {
      nomadic: {
        melee: 0.3, ranged: 0.8, mounted: 3, machinery: 0.4, naval: 1.0, armored: 1.6, aviation: 1, magical: 0.5,
      },
      wetland: {
        melee: 1, ranged: 1.6, mounted: 0.2, machinery: 1.2, naval: 1.0, armored: 0.2, aviation: 0.5, magical: 0.5,
      },
      highland: {
        melee: 1.2, ranged: 2, mounted: 0.3, machinery: 3, naval: 1.0, armored: 0.8, aviation: 0.3, magical: 2,
      },
    };
    ...
    for (const i of cells.i) {
      ...
      for (const unit of options.military) {
        ...
        stateObj.temp.platoons.push({
          cell: i, a: total, t: total, x, y, u: unit.name, n, s: unit.separate, type: unit.type,
        });
      }
    }
    ...
    for (const b of pack.burgs) {
      ...
      for (const unit of options.military) {
        ...
        stateObj.temp.platoons.push({
          cell: b.cell, a: total, t: total, x, y, u: unit.name, n, s: unit.separate, type: unit.type,
        });
      }
    }
    ...
    function createRegiments(nodes, s) {
      ...
      const regiments = nodes
        ...
        .map((r, i) => {
          ...
          return {
            i, a: r.t, cell: r.cell, x: r.x, y: r.y, bx: r.x, by: r.y, u, n: r.n, name, state: s.i,
          };
        });
  ...
  const drawRegiment = function (reg, s) {
    ...
    const g = army
      ...
      .attr('data-state', s)
      .attr('data-id', reg.i);
    g.append('rect').attr('x', x1).attr('y', y1).attr('width', w)
      .attr('height', h);
  ...
  return {
    generate, getDefaultOptions, getName, generateNote, drawRegiments, drawRegiment, moveRegiment, getTotal, getEmblem,
  };
}());
