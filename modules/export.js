...
async function getMapURL(type, options = {}) {
  const {
    debug = false, globe = false, noLabels = false, noWater = false, fullMap = false,
  } = options;
  ...
  if (fontsToLoad.length) {
    ...
    const fontFaces = dataURLfonts
      .map(({
        family, src, unicodeRange = '', variant = 'normal',
      }) => `@font-face {font-family: "${family}"; src: ${src}; unicode-range: ${unicodeRange}; font-variant: ${variant};}`)
      .join('\n');
...
function saveGeoJSON_Cells() {
  ...
  cells.i.forEach((i) => {
    ...
    const { type } = pack.features[cells.f[i]];
    ...
    const properties = {
      id: i, height, biome, type, population, state, province, culture, religion, neighbors,
    };
...
function saveGeoJSON_Rivers() {
  ...
  rivers.selectAll('path').each(function () {
    ...
    const feature = {
      ...
      properties: {
        id, i, basin, name, type, width, increment,
      },
    };
    json.features.push(feature);
  });
...
function saveGeoJSON_Markers() {
  const features = pack.markers.map((marker) => {
    const {
      i, type, icon, x, y, size, fill, stroke,
    } = marker;
    const coordinates = getQGIScoordinates(x, y);
    const id = `marker${i}`;
    const note = notes.find((note) => note.id === id);
    const properties = {
      id, type, icon, ...note, size, fill, stroke,
    };
