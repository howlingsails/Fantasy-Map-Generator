window.Markers = (function () {
  ...
  function addVolcanoes(type, icon, multiplier) {
    ...
    const mountains = Array.from(cells.i.filter((i) => !occupied[i] && cells.h[i] >= 70).sort((a, b) => cells.h[b] - cells.h[a]));
  ...
  function addHotSprings(type, icon, multiplier) {
    ...
    const springs = Array.from(cells.i.filter((i) => !occupied[i] && cells.h[i] > 50).sort((a, b) => cells.h[b] - cells.h[a]));
  ...
  function addMines(type, icon, multiplier) {
    ...
    const hillyBurgs = Array.from(cells.i.filter((i) => !occupied[i] && cells.h[i] > 47 && cells.burg[i]));
    let quantity = getQuantity(hillyBurgs, 1, 15, multiplier);
    if (!quantity) return;

    const resources = {
      salt: 5, gold: 2, silver: 4, copper: 2, iron: 3, lead: 1, tin: 1,
    };

    while (quantity && hillyBurgs.length) {
      const [cell] = extractAnyElement(hillyBurgs);
      const id = addMarker({
        cell, icon, type, dx: 48, px: 13,
      });
  ...
  function addInns(type, icon, multiplier) {
    ...
    const taverns = Array.from(cells.i.filter((i) => !occupied[i] && cells.h[i] >= 20 && cells.road[i] > 4 && cells.pop[i] > 10));
  ...
  function addBattlefields(type, icon, multiplier) {
    ...
    const battlefields = Array.from(cells.i.filter((i) => !occupied[i] && cells.state[i] && cells.pop[i] > 2 && cells.h[i] < 50 && cells.h[i] > 25));
  ...
  function addDungeons(type, icon, multiplier) {
    ...
    const dungeons = Array.from(cells.i.filter((i) => !occupied[i] && cells.pop[i] && cells.pop[i] < 3));
  ...
  function addSacredMountains(type, icon, multiplier) {
    ...
    const lonelyMountains = Array.from(
  ...
  function addSacredForests(type, icon, multiplier) {
    ...
    const temperateForests = Array.from(cells.i.filter((i) => !occupied[i] && cells.culture[i] && [6, 8].includes(cells.biome[i])));
  ...
  function addSacredPineries(type, icon, multiplier) {
    ...
    const borealForests = Array.from(cells.i.filter((i) => !occupied[i] && cells.culture[i] && cells.biome[i] === 9));
  ...
  function addSacredPalmGroves(type, icon, multiplier) {
    ...
    const oasises = Array.from(cells.i.filter((i) => !occupied[i] && cells.culture[i] && cells.biome[i] === 1 && cells.pop[i] > 1 && cells.road[i]));
  ...
  function addBrigands(type, icon, multiplier) {
    ...
    const roads = Array.from(cells.i.filter((i) => !occupied[i] && cells.culture[i] && cells.road[i] > 4));
    ...
    const types = {
      brigands: 4, bandits: 3, robbers: 1, highwaymen: 1,
    };
  ...
  function addPirates(type, icon, multiplier) {
    ...
    const searoutes = Array.from(cells.i.filter((i) => !occupied[i] && cells.h[i] < 20 && cells.road[i]));
  ...
  function addStatues(type, icon, multiplier) {
    const { cells } = pack;
    const statues = Array.from(cells.i.filter((i) => !occupied[i] && cells.h[i] >= 20 && cells.h[i] < 40));
  ...
  function addRuines(type, icon, multiplier) {
    const { cells } = pack;
    const ruins = Array.from(cells.i.filter((i) => !occupied[i] && cells.culture[i] && cells.h[i] >= 20 && cells.h[i] < 60));
  ...
  return {
    generate, regenerate, getConfig, setConfig,
  };
}());
