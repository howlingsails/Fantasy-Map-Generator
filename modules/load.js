    ...
    void (function parsePackData() {
      ...
      if (data[31]) {
        ...
        namesDL.forEach((d, i) => {
          ...
          nameBases[i] = {
            name: e[0], min: e[1], max: e[2], d: e[3], m: e[4], b,
          };
        });
      }
    }());
    ...
    void (function resolveVersionConflicts() {
      ...
      if (version < 1) {
        ...
        borders
          ...
          .attr('filter', null);
        stateBorders.attr('opacity', 0.8).attr('stroke', '#56566d').attr('stroke-width', 1).attr('stroke-dasharray', '2')
          .attr('stroke-linecap', 'butt');
        provinceBorders.attr('opacity', 0.8).attr('stroke', '#56566d').attr('stroke-width', 0.5).attr('stroke-dasharray', '1')
          .attr('stroke-linecap', 'butt');
        ...
        zones.attr('opacity', 0.6).attr('stroke', null).attr('stroke-width', 0).attr('stroke-dasharray', null)
          .attr('stroke-linecap', 'butt');
        addZones();
        ...
        fogging = viewbox.insert('g', '#ruler').attr('id', 'fogging-cont').attr('mask', 'url(#fog)').append('g')
          .attr('id', 'fogging')
          .style('display', 'none');
        fogging.append('rect').attr('x', 0).attr('y', 0).attr('width', '100%')
          .attr('height', '100%');
        defs.append('mask').attr('id', 'fog').append('rect').attr('x', 0)
          .attr('y', 0)
          .attr('width', '100%')
          .attr('height', '100%')
          .attr('fill', 'white');
      ...
      if (version < 1.1) {
        ...
        if (!document.getElementById('sinkhole')) {
          ...
          coastline.select('#sea_island').attr('opacity', 0.5).attr('stroke', '#1f3846').attr('stroke-width', 0.7)
      ...
      if (version < 1.11) {
        ...
        terrs.attr('scheme', 'bright').attr('terracing', 0).attr('skip', 5).attr('relax', 0)
          .attr('curve', 0);
      ...
      if (version < 1.21) {
        ...
        rivers.selectAll('path').each(function () {
          ...
          const type = length < 25 ? rw({
            Creek: 9, River: 3, Brook: 3, Stream: 1,
          }) : 'River';
          pack.rivers.push({
            i, parent: 0, length, source, mouth, basin: i, name, type,
          });
        });
      }
      ...
      if (version < 1.3) {
        ...
        options = {
          winds, year, era, eraShort, military,
        };
      ...
      if (version < 1.4) {
        ...
        ice.attr('opacity', null).attr('fill', '#e8f0f6').attr('stroke', '#e8f0f6').attr('stroke-width', 1)
