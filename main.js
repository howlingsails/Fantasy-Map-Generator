...
const compass = viewbox.append("g").attr("id", "compass");
...
const zones = viewbox.append("g").attr("id", "zones").style("display", "none");
...
const ice = viewbox.append("g").attr("id", "ice").style("display", "none");
...
// main data variables
const grid = {}; // initial grapg based on jittered square grid and data
const pack = {}; // packed graph and data
let seed;
let mapId;
const mapHistory = [];
let elSelected;
...
const biomesData = applyDefaultBiomesSystem();
const nameBases = Names.getNameBases(); // cultures-related data

const color = d3.scaleSequential(d3.interpolateSpectral); // default color scheme
...
const populationRate = +document.getElementById("populationRateInput").value;
const urbanization = +document.getElementById("urbanizationInput").value;
const urbanDensity = +document.getElementById("urbanDensityInput").value;
...
function rankCells() {
  ...
  for (const i of cells.i) {
    ...
    if (flMean)
      s += normalize(cells.fl[i] + cells.conf[i], flMean, flMax) * 500; // big rivers and confluences are valued
